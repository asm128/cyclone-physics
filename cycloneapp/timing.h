...
struct TimingData {
	uint32_t				frameNumber;			// The current render frame. This simply increments. 
	uint32_t				lastFrameTimestamp;		// The timestamp when the last frame ended. Times are given in milliseconds since some undefined time. 
	uint32_t				lastFrameDuration;		// The duration of the last frame in milliseconds. 
	unsigned long			lastFrameClockstamp;	// The clockstamp of the end of the last frame.
	unsigned long			lastFrameClockTicks;	// The duration of the last frame in clock ticks. 
	bool					isPaused;				// Keeps track of whether the rendering is paused. 
	
	// Calculated data
	double					averageFrameDuration;		// This is a recency weighted average of the frame time, calculated from frame durations. 
	float					fps;						// The reciprocal of the average frame duration giving the mean fps over a recency weighted average.
