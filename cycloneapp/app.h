...
class Application {
protected:
	int32_t									Height;	// Holds the height of the application window.
	int32_t									Width;	// Holds the current width of the application window.

public:
	virtual const char*						GetTitle							()										{ return "Cyclone Demo"; }	// Gets the title of the demo for the title bar of the window. The default implementation returns a generic title.
...
protected:
	static	const uint32_t					MaxContacts							= 256;	// Holds the maximum number of contacts.
			...
			::cyclone::CollisionData		Collisions							= {};	// Holds the collision data structure for collision detection.
			::cyclone::ContactResolver		Resolver							= {MaxContacts * 8};		// Holds the contact resolver.
			double							CameraTheta							= 0;						// Holds the camera angle.
			double							CameraPhi							= 15;						// Holds the camera elevation.
			int32_t							Last_x								= 0							// Holds the position of the mouse at the last frame of a drag.
				,							Last_y								= 0
				;													
			bool							RenderDebugInfo						= true;						// True if the contacts should be rendered.
			bool							PauseSimulation						= true;						// True if the simulation is paused.
			bool							AutoPauseSimulation					= false;					// Pauses the simulation after the next frame automatically.
...
public:
	...
	inline									RigidBodyApplication				()										{ Collisions.ContactArray = Contacts; }
